          ...
          client.shifts.forEach((element, x) => {
            for (let i = 0; i < element.total; i++) {
              element.workers[i] = {
                ...
                startTime: element.startTime,
                endTime: element.endTime,
                workerId: oldshifts[x].workers[i] && oldshifts[x].workers[i].workerId ? oldshifts[x].workers[i].workerId : ''
              }
            }
...
exports.getAllTimesheets = (req, res) => {
  ...
  Timesheet.find({}).sort({ 'createdAt': -1 }).populate(['JobId_Id','clientId','shifts.workers.workerId']).exec(function(err, client) {
      if (err) {
        console.log('err',err);
      } else {
        ...
        client.save().then(client => {
          ...
          Timesheet.findById(client.timesheetId, function(err, timesheet) {
            if (!timesheet)
              ...
              console.log('timesheet', timesheet)
              timesheet.shifts = client.shifts;
